...
# Example data
data <- mtcars  # Replace with your data
...
nodes <- data.frame(
  ...
  shape = "dot",
  size = 10,
  color = ifelse(node_degrees > 1, "red", NA)  # Color for connected nodes, default for isolated
)
...
visNetwork(nodes, edges) %>%
  ...
  visOptions(
    highlightNearest = list(enabled = TRUE, degree = 1),  # Highlight nearest nodes
...
nodes <- data.frame(
  id = V(cor_graph)$name,
  label = V(cor_graph)$name,
  group = sample(1:3, vcount(cor_graph), replace = TRUE),  # Example group assignment
  title = V(cor_graph)$name,
  color = sample(c("red", "green", "blue"), vcount(cor_graph), replace = TRUE),  # Example color
  shape = "dot",
  size = 10
)
