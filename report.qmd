...
par(mfrow=c(1,2))
numerical_corelation(data, threshold = 0.6)
...
```{r}
categorical_corelation(data, 0.7)
```
...
# Exemple de DataFrame avec des variables indépendantes et une variable dépendante
df <- data.frame(
  y = rnorm(100),
  X1 = rnorm(100),
  X2 = rnorm(100),
...
# Ajuster le modèle de régression linéaire initial
model <- lm(y ~ ., data = df)
...
# Suppression des variables avec un VIF élevé (> 5 ou > 10)
while (any(vif_values > 5)) {
  df <- df[, -which.max(vif_values)]
  model <- lm(y ~ ., data = df)
  vif_values <- vif(model)
}

print("Final model with reduced multicollinearity:")
summary(model)
```
