...
## variable avec une modalité dominante dépassant 97%
 ...
 data <- data %>% 
  select( -c(Street, Utilities, Condition2, RoofMatl, Heating))
...
##  on enlève les madalités des variables cateégorielles et on les rempplace par des valeur manquantes qu'on va inputer à nouveau par la methode de miss forest.
...
library(dplyr)
library(igraph)
tools <- numerical_corelation(data, 0.4)
tools
```



-   **%IncMSE** (Mean Decrease in Accuracy) : Mesure combien l'erreur du modèle augmente lorsqu'une variable est permutée. Plus la valeur est élevée, plus la variable est importante.
...
```{r}
data <- data %>% select(-c(X1stFlrSF, X2ndFlrSF))
```
```{r}
data <- data %>% select(-c(TotalBsmtSF,	LotFrontage, 	BsmtFinSF1, BsmtUnfSF, GarageArea))
```
...
```{r}
data <- data %>% select(-c(GarageType, BldgType, HouseStyle, MSZoning, ExterQual, Exterior2nd, BsmtFinSF2, GarageCond, garage_age))
```
